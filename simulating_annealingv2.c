...
char* net_file_name = "HW2Net.net";
char* output_net_file_name = "HW2Net_output.net";
char* path_to_tcl = "/mnt/e/simannealing/simulating_annealingv2.tcl";
...
void show_graphics() {
	...
	sprintf(temp_file_name,"complete_run.net");
	printf("writing file to: %s for tcl to show graphics\n",temp_file_name);
	...
	if ((new_childpid = vfork()) < 0) {
		...
		if (execlp(path_to_tcl,path_to_tcl,temp_file_name,params,NULL) < 0)
			perror("Error: execlp(simulating_annealingv2.tcl,simulating_annealingv2.tcl,temp_file_name,temp_file_name,NULL) FAILED!\n");
		/* Never returns */
	}
}
