...
char* net_file_name = "/home/cky800/git/simulating_annealing/HW2NetComplete.net";
...
void read_net_from_file () {
    ...
    fscanf(data,"%s %d",str,&ii);
    for(i=0;i<ii;i++) {
		fscanf(data,"%d %s %lf %lf %lf",&ind,str,&x,&y,&z);
		points[i].x = x;
		points[i].y = y;
	}
    fscanf(data,"%s",str);
